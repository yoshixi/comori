...
describe('runningTodo', () => {
  describe('pickRunningTodo', () => {
    ...
    it('picks timed todo in window', () => {
      const a = todo({ id: 1, starts_at: 100, ends_at: 200 });
      expect(pickRunningTodo([a], 150)).toEqual(a);
    });

    it('uses default duration when ends_at missing', () => {
      const a = todo({ id: 1, starts_at: 100, ends_at: null });
    ...
    it('picks earliest start when two overlap', () => {
      const early = todo({ id: 2, starts_at: 100, ends_at: 300 });
      const late = todo({ id: 3, starts_at: 150, ends_at: 400 });
    ...
    it('ignores done', () => {
      const a = todo({ id: 1, starts_at: 100, ends_at: 200, done: 1 });
  ...
  describe('pickNextTimedTodo', () => {
    it('returns earliest future timed', () => {
      const t1 = todo({ id: 10, starts_at: 200 });
      const t2 = todo({ id: 11, starts_at: 150 });
      expect(pickNextTimedTodo([t1, t2], 100)).toEqual(t2);
    });

    it('returns null when none in future', () => {
      const t = todo({ id: 12, starts_at: 100 });
